...
export default function SpeakerCard({ photo, name, position, company, logo, logos, logoSize = "h-8" }: SpeakerCardProps) {
  return (
    <div className="bg-white border-2 border-gray-200 rounded-3xl p-6 hover:border-[#FECE33] transition-all hover:shadow-xl w-64 flex-shrink-0">
          ...
          {logos ? (
            logos.map((logoItem, index) => (
              <img
                ...
                src={logoItem.src}
                alt={company}
                className={`${logoItem.size || 'h-8'} object-contain`}
              />
            ))
          ) : logo ? (
